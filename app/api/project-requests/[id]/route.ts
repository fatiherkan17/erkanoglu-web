...
export async function PATCH(request: Request, { params }: RouteContext) {
  try {
    ...
    const data: { status?: string; source?: string; priority?: string; lastContactAt?: Date | null; nextFollowUpAt?: Date | null } = {};
    ...
    for (const [key, label] of [["lastContactAt", "son görüşme"], ["nextFollowUpAt", "sonraki takip"]] as const) {
      ...
      if (typeof body[key] !== "string") return NextResponse.json({ success: false, message: `Geçersiz ${label} tarihi.` }, { status: 400 });
      const value = new Date(body[key]);
      if (Number.isNaN(value.getTime())) return NextResponse.json({ success: false, message: `Geçersiz ${label} tarihi.` }, { status: 400 });
      data[key] = value;
    }
