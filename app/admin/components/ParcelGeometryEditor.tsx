  ...
  return (
    <div className="mt-5 border border-black/10 bg-white p-5">
      ...
      <div className="mt-4 overflow-hidden border border-black/10 bg-[#f8f7f2]">
        <div
          className={`relative select-none ${placing ? "cursor-crosshair" : "cursor-default"}`}
        ...
        >
          ...
          <svg className="pointer-events-none absolute inset-0 h-full w-full" viewBox="0 0 1000 1000" preserveAspectRatio="none">
            {points.length >= 2 && (
              <polyline
                points={points.map((p) => `${p.x * 1000},${p.y * 1000}`).join(" ") + (points.length >= 3 ? ` ${points[0].x * 1000},${points[0].y * 1000}` : "")}
            ...
            {points.map((p, index) => (
              <g key={`${p.x}-${p.y}-${index}`}>
                <circle cx={p.x * 1000} cy={p.y * 1000} r="15" fill="white" stroke="black" strokeWidth="4" vectorEffect="non-scaling-stroke" />
                <text x={p.x * 1000} y={p.y * 1000 + 5} textAnchor="middle" fontSize="18" fontWeight="600" fill="black">{index + 1}</text>
              </g>
            ))}
          </svg>
        </div>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <button type="button" onClick={() => { setPlacing(true); setError(""); }} className={`border px-4 py-3 text-[10px] font-semibold tracking-[0.12em] ${placing ? "border-black bg-black text-white" : "border-black/15 bg-white text-black/55"}`}>
          {placing ? "NOKTALARI İŞARETLE" : "PARSEL SINIRINI İŞARETLE"}
        </button>
