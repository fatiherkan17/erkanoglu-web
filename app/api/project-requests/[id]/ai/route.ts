...
const allowedKeys: (keyof ParcelData)[] = [
  "parcelAreaM2", "emsal", "taks", "kaks", "floors", "frontageM", "depthM",
  "buildingSetbackFrontM", "buildingSetbackSideM", "buildingSetbackRearM", "roadWidthM",
  "zoningPlan", "notes",
];
...
export async function POST(request: Request, { params }: RouteContext) {
  try {
    ...
    const record = await prisma.projectRequest.findUnique({ where: { id }, select: { buildingType: true, projectStage: true, approximateArea: true, interestAreas: true, description: true } });
    ...
    if (missing.length > 0) return NextResponse.json({ success: false, message: "En az parsel alanı, emsal, TAKS ve kat adedi girilmelidir." }, { status: 400 });
    ...
    const updated = await prisma.projectRequest.update({ where: { id }, data: { aiDraft: aiDraft as Prisma.InputJsonValue }, select: { id: true, parcelData: true, aiDraft: true } });
    return NextResponse.json({ success: true, data: updated });
  } catch (error) {
